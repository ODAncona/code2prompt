...
```sh
code2prompt путь/к/кодовой/базе --exclude="*.npy,*.wav"
```

Отображение количества токенов сгенерированного промпта:

```sh
code2prompt путь/к/кодовой/базе --token-format=format
```

Указание токенизатора для подсчёта токенов:

```sh
code2prompt путь/к/кодовой/базе --encoding=p50k
```
...
```sh
code2prompt путь/к/кодовой/базе --line-numbers
```
